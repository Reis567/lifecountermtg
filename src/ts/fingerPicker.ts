...
let fpOverlay: HTMLDivElement | null = null;
let fpPointers = new Map<number, FpPointer>();
let fpTarget = 4;
let fpCountdownTimer: number | null = null;
let fpResolved = false;

const FP_COUNTDOWN_MS = 2500;

function fpRandomInt(max: number): number {
...
function fpColorFor(index: number): string {
    return DEFAULT_PLAYER_COLORS[index % DEFAULT_PLAYER_COLORS.length];
}

export function openFingerPicker(): void {
...
function fpPositionDot(el: HTMLDivElement, x: number, y: number): void {
    el.style.left = `${x}px`;
    el.style.top = `${y}px`;
}

function fpIsControl(target: EventTarget | null): boolean {
    return target instanceof HTMLElement && target.closest('.fp-close, .fp-btn') !== null;
}

function fpOnDown(e: PointerEvent): void {
    if (fpResolved || !fpOverlay) return;
    if (fpIsControl(e.target)) return;
...
function fpOnMove(e: PointerEvent): void {
    const p = fpPointers.get(e.pointerId);
    if (!p) return;
    fpPositionDot(p.el, e.clientX, e.clientY);
}

function fpOnUp(e: PointerEvent): void {
    if (fpResolved) return;
    const p = fpPointers.get(e.pointerId);
    if (!p) return;
    p.el.remove();
...
function fpOnPointersChanged(): void {
    ...
    if (fpPointers.size === fpTarget && fpTarget >= 1) {
        ...
        if (status) status.textContent = 'Não tirem os dedos... sorteando!';
