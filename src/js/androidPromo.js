...
export function maybeShowAndroidPromo() {
    try {
        if (location.protocol === 'file:')
            return; // rodando dentro do nosso APK (WebView)
        const ua = navigator.userAgent || '';
        if (!/Android/i.test(ua))
            return; // só Android
        if (/; wv\)/.test(ua))
            return; // já é uma WebView de app
...
export function setupApkDownloadButton() {
    const btn = document.getElementById('apk-download-home');
    if (!btn)
        return;
    if (location.protocol === 'file:') {
...
//# sourceMappingURL=androidPromo.js.map